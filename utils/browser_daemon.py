...
class ChromeDaemonManager:
    ...
    def surgical_kill(self) -> None:
        ...
        """
        if psutil is None:
            log.dual_log(tag="Browser:Daemon:Kill", message="psutil not available, skipping surgical kill", level="WARNING", payload={"action": "surgical_kill_skipped"})
            return
        ...
        for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
            ...
            try:
                ...
                if "chrome" in proc_name and target_dir in cmdline:
                    proc.kill()
                    killed = True
                    log.dual_log(tag="Browser:Daemon:Kill", message=f"Killed Chrome PID {current_pid} for profile {target_dir}", payload={"pid": current_pid, "profile": target_dir})
            except (psutil.AccessDenied, psutil.PermissionError) as e:
                log.dual_log(tag="Browser:Daemon:Kill", message=f"FATAL: Permission denied killing Chrome PID {current_pid}: {e}", level="CRITICAL", payload={"pid": current_pid, "error": str(e)})
                raise RuntimeError(f"Permission denied killing Chrome PID: {e}")
            except psutil.NoSuchProcess:
                continue
            except Exception as e:
                log.dual_log(tag="Browser:Daemon:Kill", message=f"Error killing process: {e}", level="WARNING", payload={"error": str(e)})
        
        if killed:
            log.dual_log(tag="Browser:Daemon:Kill", message=f"Surgically killed Chrome processes for {target_dir}", payload={"profile": target_dir, "killed": True})
    ...
    def _init_driver(self) -> Driver:
        ...
        try:
            if hasattr(self._driver, '_browser') and hasattr(self._driver._browser, '_process_pid'):
                ...
                log.dual_log(tag="Browser:Daemon:Spawn", message=f"Chrome spawned with PID {self._pid}", payload={"pid": self._pid})
            elif hasattr(self._driver, 'browser') and hasattr(self._driver.browser, 'process'):
                self._pid = self._driver.browser.process.pid
                log.dual_log(tag="Browser:Daemon:Spawn", message=f"Chrome spawned with PID {self._pid}", payload={"pid": self._pid})
            else:
                log.dual_log(tag="Browser:Daemon:Spawn", message="Chrome PID unavailable (driver structure unexpected)", level="WARNING", payload={"pid": None})
        except Exception as e:
            log.dual_log(tag="Browser:Daemon:Spawn", message=f"Failed to capture Chrome PID: {e}", level="WARNING", payload={"error": str(e)})
            self._pid = None
        ...
        if self._driver:
            log.dual_log(tag="Browser:Daemon:Settle", message="Waiting 5s for Chrome CDP to settle...", payload={"delay_s": 5})
    ...
    def get_or_create_driver(self) -> Driver:
        ...
        with self._lock:
            if self._driver is None or not self.is_driver_alive():
                log.dual_log(tag="Browser:Daemon:Init", message="Initialising new Driver session.", payload={"status": "INITIALIZING"})
                if self._driver is not None:
                    ...
                    try:
                        ...
                        log.dual_log(tag="Browser:Daemon:Shutdown", message="driver.close() timed out; forcing process kill", level="WARNING", payload={"action": "surgical_kill"})
                        self.surgical_kill()
                    except Exception as e:
                        log.dual_log(tag="Browser:Daemon:Shutdown", message=f"Error closing old driver: {e}", level="WARNING", payload={"error": str(e)})
                    finally:
        ...
        with self._lock:
            if self._driver is not None:
                try:
                    ...
                    log.dual_log(tag="Browser:Daemon:Shutdown", message=f"Error closing driver: {e}", level="WARNING", payload={"error": str(e)})
                self._driver = None
                self._status = BrowserStatus.CRITICAL_FAILURE
                log.dual_log(tag="Browser:Daemon:Shutdown", message="Driver shut down.", payload={"status": "CRITICAL_FAILURE"})
    ...
    def deep_warmup(self) -> bool:
        ...
        try:
            ...
            log.dual_log(tag="Startup:Browser:Warmup", message="Phase 0: Tab Management Test", payload={"phase": "tab_management"})
            try:
                log.dual_log(tag="Browser:Navigation:Request", message=f"Navigating to: https://google.com", payload={"url": "https://google.com"})
                ...
                log.dual_log(tag="Startup:Browser:Warmup", message=f"Tab test warning: {e}", level="WARNING", payload={"error": str(e)})
            
            # Phase 1: Navigation Test
            log.dual_log(tag="Startup:Browser:Warmup", message="Phase 1: Navigation Test", payload={"phase": "navigation"})
            ...
            # Phase 2: SoM Test
            log.dual_log(tag="Startup:Browser:Warmup", message="Phase 2: SoM Injection Test", payload={"phase": "som_injection", "tracking_keys": list(self._id_tracking.keys())})
            try:
                reinject_all(driver, self._id_tracking)
            except Exception as e:
                from utils.observation_adapter import MarkingError
                if isinstance(e, MarkingError):
                    log.dual_log(tag="Startup:Browser:Warmup", message="CRITICAL: JS Execution hung during SoM injection. Forcing surgical kill to prevent thread leak.", level="CRITICAL", payload={"action": "surgical_kill"})
                    self.surgical_kill()
                    self._status = BrowserStatus.CRITICAL_FAILURE
            ...
            # Phase 3: Vision Test
            log.dual_log(tag="Startup:Browser:Warmup", message="Phase 3: Vision Subsystem Test", payload={"phase": "vision_subsystem", "driver_alive": self.is_driver_alive()})
            ...
            log.dual_log(tag="Startup:Browser:Warmup", message="Deep Warmup Successful", payload={"status": "SUCCESS"})
            # Set READY only after full verification
            self._status = BrowserStatus.READY
            return True
        except Exception as e:
            log.dual_log(tag="Startup:Browser:Warmup", message=f"CRITICAL: Warmup Failed: {e}", level="CRITICAL", payload={"error": str(e)})
            self._status = BrowserStatus.CRITICAL_FAILURE
            return False
