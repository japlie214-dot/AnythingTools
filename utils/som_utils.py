...
def inject_som(driver: Driver, start_id: int = 1) -> int:
    ...
    try:
        ...
        log.dual_log(tag="SoM:Core:Inject", message=f"SoM injection failed entirely: {e}", level="ERROR", payload={"error": str(e)})
        return start_id
...
def remove_overlays(driver: Driver) -> None:
    ...
    try:
        ...
        log.dual_log(tag="SoM:Core:Overlays", message=f"Failed to remove overlays: {e}", level="WARNING", exc_info=e, payload={"error": str(e)})
...
def read_visible_html(driver: Driver) -> str:
    ...
    try:
        ...
        log.dual_log(tag="SoM:Core:ReadHTML", message=f"Failed to read HTML: {e}", level="WARNING", exc_info=e, payload={"error": str(e)})
        return ""
...
def get_element_by_hint(driver: Driver, hint: str) -> Tuple[str | None, str | None]:
    ...
    try:
        ...
        log.dual_log(tag="SoM:Core:Locator", message=f"Locator failed for hint '{hint}': {e}", level="WARNING", exc_info=e, payload={"hint": hint, "error": str(e)})
    return None, None
...
def enforce_single_tab(driver: Driver) -> None:
    ...
    try:
        ...
        log.dual_log(tag="SoM:Core:TabPolicy", message=f"Failed to enforce single-tab policy: {e}", level="WARNING", payload={"error": str(e)})
...
def setup_dom_stability_observer(driver: Driver) -> None:
    ...
    try:
        ...
        log.dual_log(tag="SoM:Core:Observer", message=f"Failed to set up DOM observer: {e}", level="WARNING", exc_info=e, payload={"error": str(e)})
