...
async def warmup_browser() -> None:
    ...
    try:
        ...
        log.dual_log(tag="Startup:Browser:Timeout", message="Browser warmup timed out.", level="ERROR", payload={"timeout_s": 60})
        raise RuntimeError("Browser warmup timed out after 90 seconds.")
    except Exception as e:
        log.dual_log(tag="Startup:Browser:Crash", message=f"Warmup crashed: {e}", level="CRITICAL", payload={"error": str(e)})
