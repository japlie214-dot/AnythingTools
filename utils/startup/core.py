...
class StartupOrchestrator:
    ...
    async def run(self, ctx: StartupContext) -> None:
        for tier in self._tiers:
            if not ctx.ok:
                log.dual_log(tag="Startup:Orchestrator", message="Skipping remaining phases due to prior failure", level="WARNING")
                break
    ...
    async def _run_step(self, ctx: StartupContext, name: str, step: StartupStep) -> None:
        try:
            log.dual_log(tag="Startup:Phase", message=f"Starting: {name}", level="INFO")
            await step()
            ctx.phases_completed.append(name)
            log.dual_log(tag="Startup:Phase", message=f"Completed: {name}", level="INFO")
        except Exception as e:
            ctx.ok = False
            ctx.failures.append(name)
            log.dual_log(tag="Startup:Phase", message=f"Failed: {name} - {e}", level="CRITICAL", exc_info=e)
