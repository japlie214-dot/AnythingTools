...
def get_mount_artifacts_step(app_instance):
    async def step():
        artifacts_path = getattr(config, "ARTIFACTS_DIR", "data/artifacts")
        ...
        if app_instance:
            # StaticFiles mount for artifact URL resolution (artifact_url_from_request).
        ...
        log.dual_log(tag="Startup:Server:MountArtifacts", message=f"Artifacts directory mounted at {artifacts_dir}", level="INFO", payload={"path": str(artifacts_dir)})
    return step
