...
from database.backup.schema import TABLE_SCHEMAS, validate_embedding_bytes
from database.backup.models import Watermark, ExportResult
...
def write_table_batch(table_name: str, chunks_iter, config: BackupConfig) -> int:
    ...
    try:
        ...
        if writer is not None:
            ...
            log.dual_log(tag="Backup:Storage:Write", level="INFO", message=f"Wrote {total_written} rows to {dest.name}", payload={"rows_written": total_written, "destination": str(dest)})
