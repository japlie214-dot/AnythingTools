...
from database.backup.models import RestoreResult
...
def _build_insert_sql(table_name: str, desired_cols: List[Dict[str, Any]], file_cols: List[str]) -> Optional[tuple[str, List[str]]]:
    ...
    if missing_pk:
        log.dual_log(tag="Backup:Restore:Skip", level="WARNING", message=f"Skipping {table_name}: missing PK columns {missing_pk}", payload={"table": table_name, "missing_pk": missing_pk})
        return None
    ...
    if missing_required:
        log.dual_log(tag="Backup:Restore:Skip", level="WARNING", message=f"Skipping {table_name}: missing required NOT NULL columns {missing_required}", payload={"table": table_name, "missing_required": missing_required})
        return None
...
def restore_master_tables_direct(conn: sqlite3.Connection, table_names: Optional[List[str]] = None) -> RestoreResult:
    ...
    for table_name in table_names:
        ...
        if not latest_file:
            log.dual_log(tag="Backup:Restore:Info", level="INFO", message=f"No backup data for {table_name}", payload={"table": table_name, "available": False})
            continue

        desired_cols = _get_columns(conn, table_name)
        if not desired_cols:
            log.dual_log(tag="Backup:Restore:Error", level="WARNING", message=f"Table {table_name} does not exist; cannot restore", payload={"table": table_name, "exists": False})
            continue
        ...
        try:
            ...
            log.dual_log(tag="Backup:Restore:Error", level="ERROR", message=f"Failed to read Parquet file for {table_name}: {e}", payload={"table": table_name, "file": str(latest_file) if latest_file else None, "error": str(e)})
            continue
