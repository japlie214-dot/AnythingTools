...
def execute_read_sql(sql: str, params: tuple = (), ensure_fresh: bool = False, allow_large_blobs: bool = False) -> List[Dict[str, Any]]:
    ...
    if ensure_fresh:
        try:
            ...
            try:
                ...
                log.dual_log(tag="DB:Reader:Wait", message=f"ensure_fresh wait_for_writes failed: {e}", level="WARNING", exc_info=e, payload={"error": str(e)})
    ...
    try:
        ...
        log.dual_log(tag="DB:Reader:Error", message=f"execute_read_sql failed: {e}", level="ERROR", exc_info=e, payload={"sql": sql, "params": params, "error": str(e)})
